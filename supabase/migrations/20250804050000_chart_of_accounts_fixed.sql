-- =============================================
-- MIGRACIÓN CORREGIDA: PLAN DE CUENTAS Y ASIENTOS CONTABLES
-- Fecha: 4 de agosto, 2025  
-- Descripción: Sistema contable completo para activos fijos (SYNTAX FIXED)
-- =============================================
...
-- TABLA 4: Actualizar fixed_assets con cuentas contables (usando DO block para evitar errores)
DO $$
BEGIN
    -- Agregar columnas solo si no existen
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'fixed_assets' AND column_name = 'asset_account_code') THEN
        ALTER TABLE fixed_assets ADD COLUMN asset_account_code VARCHAR(20);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'fixed_assets' AND column_name = 'depreciation_account_code') THEN
        ALTER TABLE fixed_assets ADD COLUMN depreciation_account_code VARCHAR(20);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'fixed_assets' AND column_name = 'expense_account_code') THEN
        ALTER TABLE fixed_assets ADD COLUMN expense_account_code VARCHAR(20);
    END IF;
END
$$;
...
-- Agregar foreign keys DESPUÉS de poblar datos (evita errores de referencia)
DO $$
BEGIN
    -- Solo agregar constraints si no existen
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_asset_account') THEN
        ALTER TABLE fixed_assets ADD CONSTRAINT fk_asset_account 
        FOREIGN KEY (asset_account_code) REFERENCES chart_of_accounts(code);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_depreciation_account') THEN
        ALTER TABLE fixed_assets ADD CONSTRAINT fk_depreciation_account 
        FOREIGN KEY (depreciation_account_code) REFERENCES chart_of_accounts(code);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_expense_account') THEN
        ALTER TABLE fixed_assets ADD CONSTRAINT fk_expense_account 
        FOREIGN KEY (expense_account_code) REFERENCES chart_of_accounts(code);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_parent_account') THEN
        ALTER TABLE chart_of_accounts ADD CONSTRAINT fk_parent_account 
        FOREIGN KEY (parent_code) REFERENCES chart_of_accounts(code);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_entry_details_account') THEN
        ALTER TABLE accounting_entry_details ADD CONSTRAINT fk_entry_details_account 
